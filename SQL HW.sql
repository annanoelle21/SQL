...
-- display first and last name of each actor from table

SELECT first_name, last_name FROM actor;

-- 1b. create a column named actor_name

ALTER TABLE actor
ADD actor_name VARCHAR(40);

-- 1b. display the first and last name of each actor in a sible column in uppdercase
--    in the column actor_name
...
CREATE VIEW top_tive_genres AS
SELECT c.name AS 'Genre', SUM(p.amount) AS 'Gross' 
FROM category c
JOIN film_category fc 
ON (c.category_id = fc.category_id)
...
SELECT * FROM top_tive_genres;

-- 8c. You find that you no longer need the view `top_five_genres`. 
--   Write a query to delete it.

DROP VIEW top_tive_genres;